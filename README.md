...
│   ├── utils/                   # Утилиты
│   │   └── axiosClient.js       # Настроенный HTTP клиент
│   └── middlewares/             # Промежуточные обработчики (пустая)
...
- **Модульная архитектура** - код разделен на логические модули (routes, services, config, utils)
- **Централизованная конфигурация** - все настройки в одном месте (`src/config/index.js`)
- **Валидация конфигурации** - автоматическая проверка обязательных параметров при запуске
- **Обработка ошибок** - подробные сообщения об ошибках с кодами
...
- **Логирование запросов** - подробные логи в development режиме
...
- **Обработка ошибок API** - безопасная обработка ошибок YandexGPT
...
4. **Мониторинг и логирование**:
   - Встроенное логирование запросов в development режиме
